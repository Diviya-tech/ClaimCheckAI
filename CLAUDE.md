...
## Current Phase: Weeks 7-8
Focus: Verdict engine + dossier builder + rhetorical pattern detection — the stage that evaluates each atomic fact against its retrieved evidence, assigns a categorical verdict, and assembles the final evidence dossier. Landed:
...
- `main.py` — full pipeline end to end: input -> extraction -> retrieval -> verdict evaluation -> dossier, printing the formatted dossier. `--no-evidence` still stops after extraction (debug); new `--json` also dumps the dossier as JSON (for the weeks 9-10 API). No-claim inputs still produce a (trivial) dossier for a consistent output shape.
